...
# Charger un modèle de détection de visages (Haar Cascade)
face_classifier = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")

def put_ocr_boxes(boxes, frame, height, crop_width=0, crop_height=0, view_mode=1):
    """
    Dessine des boîtes pour les textes détectés et détecte les visages dans l'image.
    """

    if view_mode not in [1, 2, 3, 4]:
        raise Exception("Un mode de vue inexistant a été sélectionné. Seuls les modes 1 à 4 sont disponibles.")

    text = ''  # Initialiser la chaîne pour concaténer le texte détecté
    
    # Convertir l'image en niveaux de gris pour la détection des visages
    gray_image = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

    # Détection des visages
    visages = face_classifier.detectMultiScale(
        gray_image, 
        scaleFactor=1.1, 
        minNeighbors=5, 
        minSize=(40, 40)
    ...
    # Dessiner des rectangles rouges autour des visages détectés
    for (x, y, w, h) in visages:
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)

    if boxes is not None:  # Assurer que Tesseract retourne des données valides
        for i, box in enumerate(boxes.splitlines()):
            box = box.split()
            if i != 0:  # Ignorer l'en-tête de la sortie Tesseract
                if len(box) == 12:  # Assurer un format de données valide
        ...
        # Classification du texte détecté
        card_type = classify_card(text)

        # Affichage du texte et du type de carte
        if text.isascii():
            cv2.putText(frame, text, (5, height - 40), cv2.FONT_HERSHEY_DUPLEX, 0.6, (200, 200, 200), 1)
            cv2.putText(frame, f"Type: {card_type}", (5, height - 5), cv2.FONT_HERSHEY_DUPLEX, 0.6, (0, 255, 0), 1)
...
def classify_card(text):
    ...
    keywords = {
        ...
        "Carte de fidélité": ["fidélité", "fidelite", "points", "client", "gratuit"]
    }

    # Nettoyer le texte pour une meilleure correspondance
    text = text.replace(" ", "").lower()

    # Calculer les scores pour chaque type de carte
    scores = {key: sum(1 for word in words if word in text) for key, words in keywords.items()}

    # Retourner le type de carte avec le score le plus élevé
    return max(scores, key=scores.get) if max(scores.values()) > 0 else "Type inconnu"
...
def ocr_stream(crop: list[int, int], source: int = 0, view_mode: int = 1, language=None):
    ...
    # Dictionary mapping Tesseract language codes to French names  
    language_mapping = {  
        'eng': 'anglais',  
        'fra': 'français',  
        'spa': 'espagnol',  
        'deu': 'allemand',  
        'ita': 'italien',  
        # Add more language codes and their French translations as needed  
    }  

    # Code to get the language name based on the Tesseract language code  
    language_code = 'fra'  # Example language code  
    lang_name = language_mapping.get(language_code, 'inconnu')  # Default to 'unknown' if code not found  

    #print(lang_name)  # Output: français
